...
var e = (e) => {
...
}, t = (e, t) => t ? typeof t == "boolean" ? e : `${e}="${t}"` : "", n = () => [{
	title: "One-Time",
	value: "one-time",
	id: "gl-freq-onetime"
}, {
	...
	className: "gl-freq-recurring"
      ...
      <label ${t("for", e.id)} ${t("class", e.className)}>
        <input type="radio" ${t("id", e.id)} name="frequency" ${t("value", e.value)} ${t("checked", n === 0)}/>
        ${e.title}
      </label>`).join("\n"), r = (e, n) => {
	let r = e.suggestedRecurringAmounts && e.suggestedRecurringAmounts.length > 1, i = e.suggestedAmounts?.map((e) => ({
		title: n.withSymbol.format(e),
		alt: n.withName.format(e),
		value: String(Math.ceil(e)),
		id: "gl-amt-" + e,
		className: r ? "gl-amt-value-onetime" : void 0
	})) || [];
	return r && e.suggestedRecurringAmounts?.forEach((e) => i?.push({
		title: n.withSymbol.format(e),
		alt: n.withName.format(e),
		value: String(Math.ceil(e)),
		id: "gl-amt-recurring" + e,
		className: "gl-amt-value-recurring"
	})), i.map((e) => `
        <label ${t("for", e.id)} ${t("class", e.className)}>
          <input type="radio" ${t("id", e.id)} name="amount" ${t("value", e.value)}/>
	...
	let i = [];
	e.suggestedAmounts && e.suggestedAmounts.length > 1 && i.push(`<fieldset class="gl-focus-container">
	...
	return e.suggestedAmounts?.length === 1 && (a = String(e.suggestedAmounts[0])), i.push(`
      <label for="gl-other-input" class="gl-other-entry">
        <span class="gl-other-label-1 sr-only">Custom amount in ${n.namePlural}</span>
        ...
        <div class="gl-focus-container">
          ...
          <input type="text" id="gl-other-input" inputmode="numeric" name="otherAmount" required ${t("value", a)} />
	...
	let n = "", r = "";
	return e.suggestedAmounts?.length === 1 && (n = t.withSymbol.format(e.suggestedAmounts[0]), r = t.withName.format(e.suggestedAmounts[0])), `
    <button type="submit" class="gl-primary">
      <span aria-hidden="true">Donate <span class="gl-donate-suffix">${n}</span></span>
      <span class="sr-only">Donate <span class="gl-donate-suffix-alt">${r}</span></span>
    </button>`;
	...
	id: "gl-ded-memory"
        ...
        <label ${t("for", e.id)}>
          <input type="radio" ${t("id", e.id)} name="ded_type" ${t("value", e.value)} ${t("checked", n === 0)}/>
          ${e.title}
        </label>`).join("\n"), c = (e) => {
	if (!e.doDedication) return "";
	let n = [
		t("data-lpignore", "true"),
		t("data-1p-ignore", !0),
		t("data-bwignore", "true"),
		t("data-protonpass-ignore", "true"),
		t("data-dashlane-disabled-on-field", "true")
	].join(" "), r = [t("data-keeper-lock", "true")].join(" ");
	return `
  <dialog id="gl-ded-modal" class="gl-ded-modal" closedby="any" aria-label="Edit Gift Dedication">
    ...
    <form novalidate ${t("aria-label", "Dedication")} ${r}>
	...
	let s = { ...r };
	s.currencyCode ||= "USD", s.locale ||= new Intl.DateTimeFormat().resolvedOptions().locale, s.ariaLabel ||= "Secure Donation";
	let u = e(s);
	return `\
<div ${t("class", "gl-simple-donation-widget" + (s.className ? " " + s.className : ""))}>
  <form novalidate
    ${t("data-slug", s.slug)}
    ${t("data-locale", s.locale)}
    ${t("data-currency", s.currencyCode)}
    ${t("aria-label", s.ariaLabel)}
  >
