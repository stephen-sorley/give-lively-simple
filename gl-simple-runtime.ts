(() => {
  ...
  const WIDGET_ROOT = ".gl-simple-donation-widget";
  
  // Shortcutsfor querySelector calls (mostly nice just to prevent having to write as HTMLElement | null endlessly in Typescript).
  ...
  if (!supports_invoker || !supports_closedby || isSafari) {
    document.querySelectorAll<HTMLDialogElement>(`${WIDGET_ROOT} dialog`).forEach(dialog => {
      ...
      if (!supports_invoker && dialog.id) {
        ...
        const invokers = document.querySelectorAll(`${WIDGET_ROOT} [commandfor="${dialog.id}"]`);
  ...
  // Loop over each simple donation widget on the page.
  document.querySelectorAll(WIDGET_ROOT).forEach( widget => {
    const qswidget = (query: string) => qs(widget, query);

    const donateForm = qswidget("form") as HTMLFormElement;
    if (!donateForm) {
      console.error("Can't find Give Lively donation widget elements - did you add the HTML to your site?");
      return;
    }

    const customInput = qswidget(".gl-other-input input") as HTMLInputElement | null;
    const amountSet = qswidget(".gl-amount fieldset");
    const amountErr = qswidget(".gl-amount-err");

    const firstOnetime = qswidget(".gl-amt-value-onetime input") as HTMLInputElement | null;
    const firstRecurring = qswidget(".gl-amt-value-recurring input") as HTMLInputElement | null;

    const dedButton = qswidget(".gl-ded-button");
    
    const donateModal = qswidget(".gl-donate-modal") as HTMLDialogElement | null;
    const dedModal = qswidget(".gl-ded-modal") as HTMLDialogElement | null;

    const iframe = qs(donateModal, "iframe") as HTMLIFrameElement | null;

    const dedForm = qs(dedModal, "form") as HTMLFormElement | null;

    const dedNameInput = qswidget(".gl-ded-name input");
    const dedNameErr = qswidget(".gl-ded-name-err");
    const dedEmailInput = qswidget(".gl-ded-email input");
    const dedEmailErr = qswidget(".gl-ded-email-err");
    ...
    const updateDonateButton = (): void => {
      ...
      qswidget(".gl-donate-suffix")?.replaceChildren(...(
        amountStr? [currencyFormatter.format(amountStr as unknown as number) + freq] : []
      ));

      qswidget(".gl-donate-suffix-alt")?.replaceChildren(...(
    ...
    if (firstOnetime && firstRecurring) {
      qswidget(".gl-frequency")?.addEventListener("change", (e) => {
